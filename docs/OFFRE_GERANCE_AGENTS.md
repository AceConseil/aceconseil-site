...
| Modèles | Claude sur la clé API du client | 15 à 30 euros/mois en usage modéré, 80 à 110 en intensif. **Deux échéances vérifiées** : le tarif de Sonnet 5 passe de 2/10 à 3/15 dollars par million de jetons le 1er septembre 2026, et le tokenizer des modèles récents produit environ 30 % de jetons en plus pour le même texte. Budgéter au tarif plein et mesurer la consommation réelle pendant le rodage avant de figer un prix |
...
| Résidence des données | **Hébergement en France** (VPS OVH au nom du client) ; **inférence hors UE** | Distinction à énoncer telle quelle, jamais à confondre : les fichiers, la mémoire et les sauvegardes sont en France et c'est vérifiable ; le texte envoyé au modèle est traité hors de l'Union européenne, l'API Claude de première main étant globale par défaut et n'offrant pas d'option européenne (seule l'option « États-Unis » existe, avec majoration). Si un client exige l'inférence en Europe, deux voies à instruire : les endpoints régionaux de Claude servis par Google Cloud ou par Amazon Bedrock, avec environ 10 % de majoration |
